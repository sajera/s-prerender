...
chrome.setUpEvents = async function (tab) {
    ...
    Network.requestWillBeSent((params) => {
        ...
        if (tab.prerender.logRequests || this.options.logRequests) console.log('+', tab.prerender.numRequestsInFlight, params.request.url);

        if (!tab.prerender.initialRequestId) {
            console.log(`Initial request to ${params.request.url}`);
    ...
    Network.loadingFinished(({ requestId, timestamp, encodedDataLength }) => {
        ...
        if (request) {
            ...
            if (tab.prerender.logRequests || this.options.logRequests) console.log('-', tab.prerender.numRequestsInFlight, tab.prerender.requests[requestId]);
        ...
        Page.enable()
            .then(() => {
                ...
                Page.navigate({
                    ...
                    console.log('invalid URL sent to Chrome:', tab.prerender.url);
                    tab.prerender.statusCode = 504;
                    finished = true;
                    reject();
                });
